...
#define FOR_BLOCKS(length, dst, src, blocksize)                                \
  ...
  for (; (length);                                                             \
       ((length) -= (blocksize), (dst) += (blocksize), (src) += (blocksize)))
...
#define SUBBYTE(x, box)                                                        \
  ((uint32_t)(box)[B0(x)] | ((uint32_t)(box)[B1(x)] << 8) |                    \
   ((uint32_t)(box)[B2(x)] << 16) | ((uint32_t)(box)[B3(x)] << 24))

#define AES_ROUND(T, w0, w1, w2, w3, k)                                        \
  ((T->table[0][B0(w0)] ^ T->table[1][B1(w1)] ^ T->table[2][B2(w2)] ^          \
    T->table[3][B3(w3)]) ^                                                     \
   (k))
...
void _aes_set_key(unsigned nr, unsigned nk, uint32_t *subkeys,
                  const uint8_t *key) {
...
void aes_set_encrypt_key(struct aes_ctx *ctx, size_t keysize,
                         const uint8_t *key) {
  unsigned nk, nr;
...
void aes_set_decrypt_key(struct aes_ctx *ctx, size_t keysize,
                         const uint8_t *key) {
...
void _nettle_aes_encrypt(unsigned rounds, const uint32_t *keys,
                         const struct aes_table *T, size_t length, uint8_t *dst,
                         const uint8_t *src) {
...
void aes_encrypt(const struct aes_ctx *ctx, size_t length, uint8_t *dst,
                 const uint8_t *src) {
  assert_beebs(!(length % AES_BLOCK_SIZE));
  _nettle_aes_encrypt(ctx->rounds, ctx->keys, &_aes_encrypt_table, length, dst,
                      src);
}

// From nettle/aes-decrypt.c

void aes_decrypt(const struct aes_ctx *ctx, size_t length, uint8_t *dst,
                 const uint8_t *src) {
  assert_beebs(!(length % AES_BLOCK_SIZE));
  _nettle_aes_decrypt(ctx->rounds, ctx->keys, &_aes_decrypt_table, length, dst,
                      src);
}

// BEEBS benchmark code

unsigned char key[32] = {0xC9, 0x95, 0x55, 0x31, 0xA6, 0x50, 0xD1, 0x54,
                         0x7B, 0xD8, 0xC5, 0xAA, 0xA4, 0xA4, 0xBD, 0xAC,
                         0x5C, 0xEA, 0x12, 0xC7, 0xF8, 0x83, 0xD6, 0xB8,
