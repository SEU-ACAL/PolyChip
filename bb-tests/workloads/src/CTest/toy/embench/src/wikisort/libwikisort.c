...
/* structure to test stable sorting (index will contain its original index in
...
void BlockSwap(Test array[], const long start1, const long start2,
...
void Rotate(Test array[], const long amount, const Range range, Test cache[],
  ...
  /* if the smaller of the two ranges fits into the cache, it's *slightly*
   * faster copying it there and shifting the elements over */
...
void WikiMerge(Test array[], const Range buffer, const Range A, const Range B,
  ...
  if (Range_length(A) <= cache_size) {
     ...
     * value that's already in that spot */
    /* when this algorithm is finished, 'buffer' will contain its original
...
void WikiSort(Test array[], const long size, const Comparison compare) {
  ...
  /* just keep in mind that making it too small ruins the point (nothing will
   * fit into it), */
  /* and making it too large also ruins the point (so much for "low memory"!) */
  /* removing the cache entirely still gives 70% of the performance of a
   * standard merge */
...
#define CACHE_SIZE 512
  ...
  /* (this is essentially fixed-point math, where we manually check for and
   * handle overflow) */
  ...
  /* first insertion sort everything the lowest level, which is 16-31 items at a
   * time */
  decimal = 0;
  fractional = 0;
  while (decimal < size) {
  ...
  /* then merge sort the higher levels, which can be 32-63, 64-127, 128-255,
   * etc. */
  for (merge_size = 16; merge_size < power_of_two; merge_size += merge_size) {
    ...
    /* as an optimization, we really only need to pull out an internal buffer
     * once for each level of merges */
    /* after that we can reuse the same buffer over and over, then redistribute
    ...
    while (decimal < size) {
      ...
      if (compare(array[end - 1], array[start])) {
        /* the two ranges are in reverse order, so a simple rotation should fix
         * it */
        Rotate(array, mid - start, MakeRange(start, end), cache, cache_size);
      } else if (compare(array[mid], array[mid - 1])) {
        Range bufferA, bufferB, buffer1, buffer2, blockA, blockB, firstA, lastA,
            lastB;
        long indexA, minA, findA;
        Test min_value;

        /* these two ranges weren't already in order, so we'll need to merge
         * them! */
        ...
        if (Range_length(level1) > 0) {
          ...
          /* the first item is always going to be the first unique value, so
           * let's start searching at the next index */
          count = 1;
          ...
          /* if the size of each block fits into the cache, we only need one
           * buffer for tagging the A blocks */
          /* this is because the other buffer is used as a swap space for
           * merging the A blocks into the B values that follow it, */
          /* but we can just use the cache as the buffer instead. this skips
           * some memmoves and an insertion sort */
          if (buffer_size <= cache_size) {
            ...
            /* the first item of the second buffer isn't guaranteed to be the
             * first unique value, so we need to find the first unique item too
             */
            count = 0;
            ...
            if (Range_length(buffer2) == buffer_size) {
              ...
              bufferA =
                  MakeRange(buffer2.start, buffer2.start + buffer_size * 2);
              bufferB = MakeRange(B.end, B.end);
              buffer1 = MakeRange(A.start, A.start + buffer_size);
              buffer2 =
                  MakeRange(A.start + buffer_size, A.start + buffer_size * 2);

            } else if (Range_length(buffer1) == buffer_size) {
              /* we found enough values for one buffer in A, so we'll need to
               * find one buffer in B */
              bufferA = MakeRange(buffer1.start, buffer1.start + buffer_size);
              buffer1 = MakeRange(A.start, A.start + buffer_size);

              /* like before, the last value is guaranteed to be the first
               * unique value we encounter, so we can start searching at the
               * next index */
              count = 1;
              ...
              /* we were unable to find a single buffer in A, so we'll need to
               * find two buffers in B */
              count = 1;
              for (buffer1.start = B.end - 2; buffer1.start >= B.start;
                   buffer1.start--)
                if (compare(array[buffer1.start], array[buffer1.start + 1]) ||
          ...
          if (Range_length(buffer1) < buffer_size) {
            /* we failed to fill both buffers with unique values, which implies
             * we're merging two subarrays with a lot of the same values
             * repeated */
            /* we can use this knowledge to write a merge operation that is
             * optimized for arrays of repeating values */
            while (Range_length(A) > 0 && Range_length(B) > 0) {
              ...
              Rotate(array, -amount, MakeRange(A.start, mid), cache,
                     cache_size);
          ...
          for (index = bufferA.start; count < length; index--) {
            if (index == A.start || compare(array[index - 1], array[index]) ||
              ...
              Rotate(array, -count, MakeRange(index + 1, bufferA.start + 1),
                     cache, cache_size);
        ...
        /* break the remainder of A into blocks. firstA is the uneven-sized
         * first A block */
        ...
        /* whenever we leave an A block behind, we'll need to merge the previous
         * A block with any B blocks that follow it, so track that information
         * as well */
        lastA = firstA;
        lastB = MakeRange(0, 0);
        blockB = MakeRange(
            B.start,
            B.start + Min(block_size, Range_length(B) - Range_length(bufferB)));
        ...
        while (true) {
           ...
           * block is <= the last value of the previous B block */
          if ((Range_length(lastB) > 0 &&
            ...
            /* we need to swap the second item of the previous A block back with
             * its original value, which is stored in buffer1 */
            /* since the firstA block did not have its value swapped out, we
             * need to make sure the previous A block is not unevenly sized */
            Swap(array[blockA.start + 1], array[buffer1.start + indexA++],
                 Test);

            /* locally merge the previous A block with the B values that follow
             * it, using the buffer as swap space */
            WikiMerge(array, buffer2, lastA, MakeRange(lastA.end, B_split),
                      compare, cache, cache_size);

            /* copy the previous A block into the cache or buffer2, since that's
             * where we need it to be when we go to merge it anyway */
            ...
            /* the area normally taken up by the A block is either the contents
             * of buffer2, or data we don't need anymore since we memcopied it
             */
            /* either way, we don't need to retain the order of those items, so
             * instead of rotating we can just block swap B to where it belongs
             */
            ...
            /* search the second value of the remaining A blocks to find the new
             * minimum A block (that's why we wrote unique values to them!) */
            minA = blockA.start + 1;
            ...
            minA =
                minA -
                1; /* decrement once to get back to the start of that A block */
            min_value = array[minA];
            ...
            /* (using the cache is disabled since we have the contents of the
             * previous A block in it!) */
            Rotate(array, -Range_length(blockB),
                   MakeRange(blockA.start, blockB.end), cache, 0);
            lastB =
                MakeRange(blockA.start, blockA.start + Range_length(blockB));
        ...
        WikiMerge(array, buffer2, lastA,
                  MakeRange(lastA.end, B.end - Range_length(bufferB)), compare,
                  cache, cache_size);
      }
    }
    ...
    if (Range_length(level1) > 0) {
      ...
      /* when we're finished with this step we should have b1 b2 left over,
       * where one of the buffers is all jumbled up */
      /* insertion sort the jumbled up buffer, then redistribute them back into
       * the array using the opposite process used for creating the buffer */
      ...
      for (index = levelA.end; Range_length(levelA) > 0; index++) {
        if (index == levelB.start ||
          ...
          Rotate(array, -amount, MakeRange(levelA.start, index), cache,
                 cache_size);
...
long TestingRandom(long index, long total) { return rand_beebs(); }
...
long TestingAscending(long index, long total) { return index; }

long TestingDescending(long index, long total) { return total - index; }

long TestingEqual(long index, long total) { return 1000; }
