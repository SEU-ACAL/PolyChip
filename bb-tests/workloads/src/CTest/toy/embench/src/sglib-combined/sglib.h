...
#define SGLIB_LIST_DELETE(type, list, elem, next)                              \
  {                                                                            \
    ...
    for (_p_ = &(list); *_p_ != NULL && *_p_ != (elem); _p_ = &(*_p_)->next)   \
      ;                                                                        \
    assert(*_p_ != NULL && "element is not member of the container, use "      \
                           "DELETE_IF_MEMBER instead" != NULL);                \
    *_p_ = (*_p_)->next;                                                       \
  }
...
#define SGLIB_DEFINE_RBTREE_FUNCTIONS_GENERAL(type, left, right, bits,         \
  ...
  int sglib___##type##_delete_recursive(type **tree, type *elem) {             \
    ...
    res = 0;                                                                   \
    if (t == NULL) {                                                           \
      assert(0 && "The element to delete not found in the tree,  use "         \
      ...
      if (cmp < 0 || (cmp == 0 && elem < t)) {                                 \
        ...
        assert(elem == t && "Deleting an element which is non member of the "  \
                            "tree, use 'delete_if_member'" != NULL);           \
        if (t->left == NULL) {                                                 \
          if (t->right == NULL) {                                              \
            /* a leaf, delete, it; */                                          \
            *tree = NULL;                                                      \
